...
from DIRAC.ConfigurationSystem.Client.Helpers.Registry import getUserOption, getAllUsers, getHosts, getHostOption, getAllGroups, getDNsInGroup
...
class RucioSynchronizer(AgentModule):
    """
    .. class::  RucioSynchronizer
            ...
            if result['OK']:
                ...
                for se in result['Value']:
                    if se not in rses:
                        ...
                        dictRSEAttributes = {'naming_convention': 'BelleII', 'ANY': True, 'fts': 'https://fts.usatlas.bnl.gov:8446'}
                        dictRSEAttributes = {'naming_convention': 'BelleII', 'ANY': True, 'fts': 'https://kek2-fts01.cc.kek.jp:8446'}
                        ...
                        if se.find('TMP-SE') > -1:
                            client.add_rse_attribute(se, 'TMP-SE', True)
                            client.set_local_account_limit(account, se, 100000000000000000)
                    ...
                    existing_protocols = []
                    for prot in protocols:
                        existing_protocols.append((str(prot['scheme']), str(prot['hostname']), str(prot['port']), str(prot['prefix'])))
                    protocols_to_create = []
                    for cnt in range(len(result['Value'][se])):
                        ...
                        if prot not in existing_protocols and prot[0] in valid_protocols:
                            ...
                            self.log.info('Will create new protocol %s://%s:%s%s on %s' % (params['scheme'], params['hostname'], params['port'], params['prefix'], se))
                            try:
                            ...
                            for protocol in protocols:
                                if prot == (str(protocol['scheme']), str(protocol['hostname']), str(protocol['port']), str(protocol['prefix'])):
                                    ...
                                    if update:
                                        ...
                                        self.log.info('Will update protocol %s://%s:%s%s on %s' % (params['scheme'], params['hostname'], params['port'], params['prefix'], se))
                                        client.update_protocols(rse=se, scheme=params['scheme'], data=data, hostname=params['hostname'], port=params['port'])
